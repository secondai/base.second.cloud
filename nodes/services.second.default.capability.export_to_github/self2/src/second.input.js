...
const inputFunc = async function({universe, SELF, INPUT, PATH}){
  ...
  let {
    ghUser,
    ghToken,
    nodePathsToWriteWithFiles, // array of array of paths: [ [internal, saveTo], internal, internal ]
    repoUrl
  } = INPUT.data;
...
// use pre-generated OAuth token for basic auth 
  ...
  for(let nodePath of nodePathsToWriteWithFiles){
    ...
    let internalNodePath,
      copyToNodePath;
    if(lodash.isArray(nodePath)){
      internalNodePath = nodePath[0];
      copyToNodePath = nodePath[1];
    } else {
      internalNodePath = nodePath;
      copyToNodePath = nodePath;
    }

    // get node data 
    let nodeData = await universe.getNodeAtPath(internalNodePath, {excludeChildren: true});
    // nodesToWrite['nodes/' + nodePath + '.json'] = JSON.stringify(nodeData, null, 2)
    let content = JSON.stringify(nodeData, null, 2);
    zip.addFile('nodes/' + copyToNodePath + '.json', Buffer.alloc(content.length, content), null); // leaving out 'entry comment' at end 
    console.log('addNode:', copyToNodePath);
    addedNodes = true;

    let cwd = path.join(universe.env.ATTACHED_VOLUME_ROOT, internalNodePath) + '/';
    // console.log('CWD:', cwd);
    // return {};
    ...
    try {
      ...
      console.error('Directory doesnt exist:', internalNodePath);
      continue;
    }
    // console.log(isDir);
    if(isDir && isDir.isDirectory()){
      let matches = await globby('*/**', fileOpts)
      console.log('File matches for', internalNodePath, ':', matches.length);
      ...
      for(let match of matches){
        ...
        try {
          ...
          zip.addFile('nodes/' + copyToNodePath + '/' + match, fileBuffer, '', parseInt('0644', 16));
          console.log('addFile:', internalFilePath, 'files/' + copyToNodePath + '/' + match);
          addedFiles = true;
      ...
      console.error('No directory for:', internalNodePath);
    }
  }
  ...
  try {
    ...
    // // Build the command
    // - THIS FAILS (on bigger directories??) 
    // var rsync = new Rsync()
    //   // .shell('ssh -p 2222')
    //   .flags('v')
    //   .recursive()
    ...
    // let fileExcludePatterns = ['/node_modules','/.DS_Store','/.git'];
    let fileExcludePatterns = ['**/node_modules','**/.DS_Store','**/.git'];
    ...
    // - kinda like syncing with rsync 
    let thingsToRemove = await globby(['*','!.git'],{
    ...
    for(let thingPath of thingsToRemove){
      try {
        await fs.remove(path.join(repoCloneToUrl, thingPath));
      }catch(err){
        console.error(err);
      }
    }
    ...
    // Copy files 
    console.log('Copying files/folders over to .git repo');
    await fs.copy( finalZipPathExtracted, repoCloneToUrl, {
    ...
    return {
      type: 'types.second.default.response.export_to_github',
      data: {
        exported: true,
    ...
    console.error('Failed exec commands', err);
    return {
      type: 'types.second.default.error',
      data: {
        error: true,
